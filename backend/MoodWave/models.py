...
class UserProfile(models.Model):
    ...
    tracks = models.ManyToManyField('Track', related_name='users', blank=True)
...
class Track(models.Model):
    ...
    # will help with rec system later
    ...
    source = models.CharField(max_length=50, default="soundcloud")
...
class TrackInteraction(models.Model):
    ...
    def __str__(self):
        return f"{self.user.username} {self.action} {self.track.spotify_id} @ {self.timestamp}"

class Playlist(models.Model):
    user_profile = models.ForeignKey(UserProfile, on_delete=models.CASCADE, related_name="playlists", null=True, blank=True)
